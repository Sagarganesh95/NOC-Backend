            ...
            for (let building of global.domainData) {
                ...
                if (hostData.length > 0) {
                    for (let h of hostData) {
                        ...
                        // let hostLogs = await hostHealthLogsCountByTime(building, fromdate, toDate, h.hostId);
                        // obj.hostlogCount  = hostLogs.map( (log)=> { if( h.hostId == log._id) { return log.count } else { return 0 } })
...
exports.hostHealthLogsCountByTime = async(subdomain, fromDate, toDate, hostName) => {
    let arrayData = 0;
    try {
        arrayData = await db[subdomain].hostLogs.countDocuments({ lastUpdated: { $gt: new Date(fromDate), $lte: new Date(toDate) }, hostId: mongoose.Types.ObjectId(hostName) });
        return arrayData;
    } catch (e) {
...
exports.unhealthyhostsData = async(req, subdomain, fromDate, toDate) => {
    ...
    try {
        ...
        if (pass == true) {
            ...
            let hostData = await db[subdomain].hostLogs.aggregate([
                { $lookup: { from: "hosts", localField: "hostId", foreignField: "_id", as: "hosts_docs" } }, { $unwind: "$hosts_docs" },
                { $sort: { lastUpdated: 1 } },
                {
                    $group: {
                        ...
                        buildingId: { $first: "$hosts_docs.buildingId" },
                        maxTime: { $max: "$lastUpdated" },
                        firstResponse: { $first: "$lastUpdated" }
                    }
                },
                { $lookup: { from: "buildings", localField: "buildingId", foreignField: "_id", as: "building_docs" } }, { $unwind: "$building_docs" },
                { $match: { "building_docs._id": buildingM } },
                { $lookup: { from: "floors", localField: "floor", foreignField: "_id", as: "floors_docs" } }, { $unwind: "$floors_docs" },
                { $project: { hostId: "$_id.hostId", firstResponse: "$firstResponse", hostName: "$hostName", maxTime: "$maxTime", buildingName: "$building_docs.alias", floorName: "$floors_docs.alias", timeZone: "$building_docs.timezone", _id: 0 } }
            ]).allowDiskUse(true);
            if (hostData.length > 0) {
                for (let h of hostData) {
                    let obj = {};
                    let startm = moment.tz(h.timezone).startOf('day').utc();
                    let endm = moment.tz(h.timezone).endOf('day').utc();
                    let momentstart = moment.tz(fromDate, h.timezone).valueOf();
                    let momentend = moment.tz(toDate, h.timezone).valueOf();
                    ...
                    obj.id = h.hostId;
                    obj.firstresponse = moment.tz(h.firstResponse, h.timezone).format("DD-M-YYYY hh:mm A");
                    obj.desk = DblesCount;
                    obj.nova = PblesCount;
                    obj.hostname = h.hostName;
                    obj.lastresponse = moment.tz(h.maxTime, h.timezone).format("DD-M-YYYY hh:mm A");
