                ...
                if (result.length > 0) {
                    ...
                    result.forEach(async function(room, i) {
                        ...
                        if (result.length == ++count) {
                            ...
                            console.log(sendArray, "FinalOutput in if")
                            var finalResult = []
                            sendArray.forEach(function(f) {
                                if (f.doorWays) {
                                    finalResult.push(f)
                    ...
                    console.log(" PCS no response from = ", req.body.subdomain);
                    console.log(sendArray, "FinalOutput in last")
                    var finalResult = []
                    sendArray.forEach(function(f) {
                        if (f.doorWays) {
                            finalResult.push(f)
...
exports.gethealthcount = async function(req, res, next) {
    try {
        ...
        if (type == "VIRGO") {
            // limit = 1440;
            // let segmentscount = await db[obj.subdomain].segmentToBles.find({ floorId: { $in: floors[0].floors } }, { bleId: 1, _id: 0 });
            let doorscount = await db[obj.subdomain].doorToBles.find({ floorId: { $in: floors[0].floors } }, { bleId: 1, _id: 0 });
            // for (let ble of segmentscount) {
            //     Blescount.push(ble)
            // }
            for (let ble of doorscount) {
                Blescount.push(ble)
            }
        }
        if (type == "DESK") {
...
exports.getMinutesData = async function(req, res, next) {
    ...
    var s = new Date(obj.FromDate),
        e = new Date(obj.ToDate)
    startm = moment.utc(moment.tz(s, obj.blgtimezone).startOf('day')),
        endm = moment.utc(moment.tz(e, obj.blgtimezone).endOf('day'));
    ...
    if (type == "host") {
        health = await db[obj.subdomain].hostLogs.aggregate([
            { $match: { hostId: mongoose.Types.ObjectId(obj.bleId), lastUpdated: { $gte: new Date(startm), $lte: new Date(endm) } } },
    ...
    finaldata.health = healthData(health, s, obj.blgtimezone, e);
    finaldata.minute = minuteData(d, s, obj.blgtimezone, e);
    finaldata.hours = HourData(d, s, obj.blgtimezone);
    ...
    console.log(" PCS response from getMinutesData = ", obj.subdomain);
